...
const required = [
  ...
  "scripts/profile-project.mjs",
  "scripts/run-checks.mjs",
  "scripts/check-native-controls.mjs",
  "references/geometry-and-ds-acceptance.md",
  "scripts/transform-ui.mjs",
  "scripts/migration-state.mjs",
  "references/automation.md",
  "templates/ui-system.config.example.json",
  "evals/run-regression-evals.mjs",
...
if (await pathExists(skillPath)) {
  ...
  if (!frontmatter) {
    ...
    for (const key of uniqueKeys) {
      if (!allowed.includes(key))
        errors.push(`Non-portable frontmatter key: ${key}`);
    }
    ...
    if (
      !/^name:\s*prototype-design-system-migrator\s*$/m.test(frontmatter[1])
    ) {
  ...
  const forbidden = [
    ...
    [
      "Claude disable-model-invocation frontmatter",
      /^disable-model-invocation\s*:/m,
    ],
    ["Claude dynamic shell injection", /!`[^`]+`/],
    ["Claude argument substitution", /\$ARGUMENTS|\$\{CLAUDE_/],
    [
      "Codex-only tool namespace dependency",
      /\b(?:web\.run|api_tool\.|container\.)/,
    ],
  ];
  for (const [label, regex] of forbidden) {
    if (regex.test(skill))
      errors.push(`Non-portable construct in SKILL.md: ${label}`);
  }

  const references = [
    ...skill.matchAll(/`((?:references|templates|scripts)\/[^`\s]+)`/g),
  ].map((match) => match[1]);
  for (const relative of references) {
    if (!(await pathExists(path.join(root, relative))))
      errors.push(`SKILL.md references missing path: ${relative}`);
  }

  const lineCount = skill.split("\n").length;
  if (lineCount > 260)
    warnings.push(`SKILL.md has ${lineCount} lines; keep it as a thin router.`);
}
