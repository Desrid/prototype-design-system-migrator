...
import { loadConfig, parseArgs, pathExists, printResult, readJson, walkFiles, normalizeRelative } from './lib.mjs';
...
const evidence = [];
const addEvidence = (kind, value, source) => evidence.push({ kind, value, source });
...
if (has('next')) {
  ...
  tier = 1;
  addEvidence('framework', framework, `package.json: react${has('vite') ? ' + vite' : ''}`);
} else if (has('nuxt')) {
  framework = 'nuxt';
  tier = 2;
  ...
  if (htmlFiles.length) {
    ...
    tier = 1;
    addEvidence('framework', 'static-html', normalizeRelative(root, htmlFiles[0]));
  }
}
...
if (!packageManagers.length && Object.keys(packageJson).length) packageManagers.push('npm-or-unknown');
...
if (sourceFiles.some((file) => /\.module\.(css|scss|sass|less)$/.test(file)) && !styling.includes('css-modules')) {
  styling.push('css-modules');
  addEvidence('styling', 'css-modules', normalizeRelative(root, sourceFiles.find((file) => /\.module\./.test(file))));
}
if (sourceFiles.length && !styling.includes('plain-css')) {
  styling.push('plain-css');
  addEvidence('styling', 'plain-css', normalizeRelative(root, sourceFiles[0]));
}
...
for (const [pkg, label, type] of systemPackages) {
  if (has(pkg) && !systems.some((item) => item.name === label)) {
    ...
    addEvidence(type === 'icons' ? 'icons' : 'component-system', label, `package.json: ${pkg}`);
  }
}
...
if (fullSystems.length === 1) {
  recommendation = {
    ...
    confidence: 'high',
    reason: 'Exactly one full component system is declared in the root package.'
  };
  ...
  recommendation = {
    ...
    system: null,
    confidence: 'high',
    reason: 'Multiple full component systems are declared; usage evidence is required before choosing an incumbent.'
  };
  ...
  recommendation = {
    ...
    system: 'shadcn-style',
    confidence: 'medium',
    reason: 'A shadcn-style components.json file exists and no competing full system was detected.'
  };
} else {
  recommendation = {
    ...
    reason: 'No coherent full component system was detected. Audit usage before proposing a new dependency.'
  };
}
...
if (args.out) await fs.writeFile(path.resolve(args.out), `${JSON.stringify(result, null, 2)}\n`, 'utf8');
